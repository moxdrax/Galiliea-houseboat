...
// ── Timing display — memoized ──────────────────────────
const TimingDisplay = memo(({ cruiseType, tier }) => {
...
// ── Fish Animations — extracted to avoid re-renders ──────────────────────────
const FishAnimations = memo(() => {
    const fishData = [
...
const FoodMenu = () => {
    const [cruiseType, setCruiseType] = useState('day');
    ...
    const changeCruiseType = (newType) => {
        if (newType !== cruiseType) setCruiseType(newType);
    };
                ...
                <div className="max-w-[1400px] mx-auto px-6">
                    ...
                    <div className="max-w-4xl mx-auto mb-8 space-y-8">
                        ...
                        <div className="flex justify-center">
                            <div className="inline-flex p-1 bg-neutral-100 dark:bg-neutral-800 rounded-full border border-primary/10">
                                <button
                                    className={`cursor-pointer cruise-selector-btn px-8 py-3 font-medium rounded-full text-[10px] tracking-[0.2em] uppercase transition-all ${cruiseType === 'day' ? 'active' : ''}`}
                                    onClick={() => changeCruiseType('day')}>
                                    Day Cruise
                                </button>
                                <button
                                    className={`cursor-pointer cruise-selector-btn px-8 py-3 font-medium rounded-full text-[10px] tracking-[0.2em] uppercase transition-all ${cruiseType === 'overnight' ? 'active' : ''}`}
                                    onClick={() => changeCruiseType('overnight')}>
                                    Overnight Cruise
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-center border-b border-primary/10">
                            <div className="flex gap-12">
                                <button
                                    className={`cursor-pointer type-selector-btn pb-4 font-medium text-[10px] tracking-[0.3em] uppercase transition-all ${isDeluxe ? 'active' : ''}`}
                    ...
                    <section className="pt-0 pb-12 bg-ivory-luxury" id="catalog-section">
                        <div className="max-w-[1000px] mx-auto px-6">
                            {/* The key here ensures the component re-animates on change with minimal lag */}
                            <div
                                key={`${cruiseType}-${tier}`}
                                className="catalog-container rounded-2xl overflow-hidden p-8 md:p-12 menu-animate-in"
                                id="menu-catalog"
                            >
                                ...
                                {/* Menu Content Grid */}
                                <div id="menu-content-grid" className="flex flex-col gap-6 max-w-2xl mx-auto text-center">
                                    {/* Welcome Drink */}
                                    ...
                                    <div className="meal-section">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <ul className="menu-item-text">
                                                    {isDay ? (
                                                        <><li>Fish Fry (Pearl Spot)</li><li>Chicken Roast</li></>
                                                    ) : (
                                                        isDeluxe ? <li>Fish Fry (Pearl Spot / Seer)</li> : <><li>Fish Curry</li><li>Chicken Fry</li></>
                                                    )}
                                                </ul>
                                            </div>
                                        </div>
                                    ...
                                    {!isDay && (
                                        <div className="meal-section">
                                            ...
                                            <div className="space-y-4">
                                                {isDeluxe ? (
                                                    <ul className="menu-item-text">
                                                        <li>Idli / Dosa</li><li>Sambar &amp; Chutney</li><li>Tea / Coffee</li>
                                                    </ul>
                                                ) : (
                                                    <>
                                                        <div>
                                ...
                                <div className="mt-10 text-center">
                                    ...
                                    <p className="text-[#5D4037] text-lg">Note: Menu items are subject to change</p>
                                </div>
                            </div>
                        </div>
                    </section>
